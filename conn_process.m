    ...
    if ~recompute
        ...
        if ~nargout||nargout>1,
            try
                ROI=ROIout;
                tinfo=whos('ROI');
                if tinfo.bytes>2e9 && ~logical(exist('OCTAVE_VERSION', 'builtin')), saveOpt{1} = '-v7.3';
                else, saveOpt{1} = '-v7';
                end
                conn_savematfile(fullfile(filepathresults2,'ROI.mat'),'ROI',saveOpt{:});
            ...
            try % adds summary info
                ...
                conn_savematfile(fullfile(filepathresults2,'ROI.mat'),'summary','-append',saveOpt{:});
            end
            try
